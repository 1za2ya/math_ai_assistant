...
diagram.typeには図の種類を簡潔に入れてください。数値座標を持つ点や線分を座標平面に描く場合は、diagram.typeを必ず"coordinate-plane"にしてください。diagram.dataのpointsには点のラベルと座標（座標を特定できない場合はnull）、segmentsには端点のラベルと辺の表示、expressionsにはグラフなどを表す式を入れてください。使わない配列は空にし、具体的な描画コードは生成しないでください。
